...
export default class TabNavigator extends React.Component {
  ...
  render() {
    let { style, search2, billingTab, signIn, tabs, search, children, tabBarStyle, tabBarShadowStyle, sceneStyle, ...props } = this.props;
    let scenes = [];
    ...
    React.Children.forEach(children, (item, index) => {
      ...
      let scene =
        <SceneContainer tabPosition={this.props.tabPosition} billingTab={this.props.billingTab} search2={this.props.search2} signIn={this.props.signIn} key={sceneKey} selected={selected} style={sceneStyle}>
          {item}
        </SceneContainer>;
    ...
    if (billingTab) {
      tabBarStyle.backgroundColor= '#F2F5FB';
      ...
      tabBarStyle.borderColor= '#2F80F6';
    }

    if (signIn) {
      tabBarStyle.backgroundColor= '#3083FF';
